...
export default function LocationPlaybackInner({ userId, userName }: LocationPlaybackProps) {
        ...
        <div className="rounded-xl border bg-card p-4 shadow-sm space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => { setPlaying(false); setCurrentIndex(0); }}>
                <RotateCcw className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => { setPlaying(false); setCurrentIndex(Math.max(0, currentIndex - 10)); }}>
                <SkipBack className="h-4 w-4" />
              </Button>
              <Button size="icon" className="h-10 w-10 rounded-full bg-cyan-500 hover:bg-cyan-600" onClick={() => setPlaying(!playing)}>
                {playing ? <Pause className="h-5 w-5" /> : <Play className="h-5 w-5 ml-0.5" />}
              </Button>
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => { setPlaying(false); setCurrentIndex(Math.min(points.length - 1, currentIndex + 10)); }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {[0.5, 1, 2, 5, 10].map((s) => (
                  <button key={s} onClick={() => setSpeed(s)} className={`rounded-md px-2 py-1 text-xs font-medium transition-colors ${speed === s ? "bg-cyan-500 text-white" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
                    {s}x
                  </button>
                ))}
              </div>
