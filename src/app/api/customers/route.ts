...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (isWalkInReuse) {
      ...
      if (existing) {
        // Same email = same Customer, but staff's latest typed name (Carl)
        // must win for the new booking and for the shared row — otherwise
        // the dashboard keeps showing stale Katrina for the new row.
        // Always refresh so both snapshot (new code) and fallback (old
        // cached JS without snapshot) show Carl. Distinct persons should use
        // distinct emails.
