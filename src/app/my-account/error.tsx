...
export default function MyAccountError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="min-h-screen bg-blue-50/50 dark:bg-background flex flex-col items-center justify-center gap-6 px-4 text-center">
      ...
      <div>
        ...
        <p className="mt-2 text-muted-foreground">{error.message || "Something went wrong while loading your account."}</p>
      </div>
